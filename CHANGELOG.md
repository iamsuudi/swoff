...
- feat: standalone `/assets` marketing page with feature cards and comparison table
- feat: landing page text gradients, professional footer, nav + frameworks list extracted to shared file
- feat: remove `status` from `usePrecacheProgress` — only `progress` remains
- fix: remove SW status gate from precache progress UI
- refactor(brand): invert assets color scheme (black-bg → white-bg)
- docs: replace "React hooks" with "Framework adapters" across all docs
