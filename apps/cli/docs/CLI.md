...
PWA assets (icons, favicons, splash screens) are generated by the `assets` command — see below.

---

## `assets`

Generates PWA assets from a source logo image as configured in `features.pwa.assets.source`.

```
swoff assets
swoff assets --source ./logo.svg
swoff assets --source ./logo.png --no-splash
```
