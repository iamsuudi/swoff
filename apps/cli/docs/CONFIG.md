...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `mode` | `"spa"` \| `"default"` \| `"network-first"` \| `"stale-while-revalidate"` \| `"ssr"` | `"spa"` | Navigation mode. `"spa"`: serve SPA fallback for unmatched navigations. `"default"`: passthrough. `"network-first"`: try network, fall back to cache/offline. `"stale-while-revalidate"`: serve cached HTML instantly, refresh in background. `"ssr"`: same as `"network-first"` with auto-prefetch that intercepts `history.pushState`/`replaceState` to warm the SW cache on client-side navigation. |
