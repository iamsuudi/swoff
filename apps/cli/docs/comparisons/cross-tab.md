...
- **Coverage:** All cache invalidation — auto-invalidation after mutations, manual `invalidateByTag`, server push invalidation (SSE/WebSocket), and mutation queue sync completion.
...
- **Server push broadcast:** When the SW receives an invalidation signal via SSE or WebSocket from the server, it broadcasts to every connected tab simultaneously — the tab holding the SSE connection doesn't need to relay through a second channel.
...
| **Survives all tabs closed?** | ✅ Yes — cache persists in Storage API, tag registry in IDB; SW updates cache directly via background sync | ❌ No (in-memory, wiped on tab close) | ❌ No (in-memory, wiped on tab close) | ❌ No (BroadcastChannel lost when all tabs close) |
...
1. **Lost on all tabs closed:** BroadcastChannel is main-thread only. If all tabs close and the SW replays mutations via Background Sync, there's no BroadcastChannel to notify — the mutated responses are cached but no tab knows to refetch. Swoff handles this at the storage layer: the mutation replays, the cache is updated directly, and when a new tab opens it reads fresh data — no message queuing needed.
...
4. **SSE/WebSocket relay:** With BroadcastChannel, the tab that holds the SSE/WebSocket connection must relay server invalidation messages through BroadcastChannel to other tabs — adding latency and a point of failure. With Swoff, the SW holds the SSE/WS connection and broadcasts directly to all tabs, eliminating the relay hop.
