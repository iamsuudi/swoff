...
No dedicated hook — server push runs entirely in the SW. Use `useCacheInvalidation` to respond to invalidation events in the UI:
...
function LiveData() {
  ...
  // SW calls invalidateByTag when the server sends tags via SSE/WS.
  // Active useCachedFetch hooks re-fetch automatically —
  // no manual wiring needed.
  return <div>Data updates automatically via server push</div>;
}
```
...
- [Tag invalidation: glob patterns, cascading](./06-tag-invalidation.md)
- [Auth guide: why cookie auth is required](./05-auth.md)
- [Push notifications: browser push vs server push](./09-push.md)
- [Config reference: features](../CONFIG.md#featuresserverpush)
