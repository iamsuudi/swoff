...
describe("buildMinimalConfig", () => {
  ...
  it("includes auth block with type when authEnabled", () => {
    ...
    expect((config.features as Record<string, unknown>).auth).toEqual({
      enabled: true,
      type: "bearer",
    });
  });
  ...
  it("includes mutationQueue when mutationEnabled", () => {
    ...
    expect((config.features as Record<string, unknown>).mutationQueue).toEqual({
      enabled: true,
    });
  });
  ...
  it("includes tagInvalidation when enabled", () => {
    ...
    expect(
      (config.features as Record<string, unknown>).tagInvalidation,
  ...
  it("includes graphql when enabled", () => {
    ...
    expect((config.features as Record<string, unknown>).graphql).toEqual({
      enabled: true,
    });
  });
  ...
  it("includes serverPush when enabled", () => {
    ...
    expect((config.features as Record<string, unknown>).serverPush).toEqual({
      enabled: true,
      type: "sse",
      endpoint: "/api/events",
    });
  ...
  it("includes strategy patterns when provided", () => {
    ...
    expect(
      (config.features as Record<string, unknown>).serviceWorker,
    ).toHaveProperty("strategy");
    const strategy = (
      (config.features as Record<string, unknown>).serviceWorker as Record<
        string,
        unknown
      >
