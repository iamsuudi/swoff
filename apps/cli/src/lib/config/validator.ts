...
/** Flat v1 keys that moved under caching in v2 — reported with a migration hint. */
const MOVED_TO_CACHING: Record<string, string> = {
  tagInvalidation: "features.caching.tagInvalidation",
  mutationQueue: "features.caching.mutationQueue",
  graphql: "features.caching.graphql",
  serverPush: "features.caching.serverPush",
  refetchQueue: "features.caching.refetchQueue",
  requestBatchWindowMs: "features.caching.requestBatchWindowMs",
};
...
export function validateConfig(config: SwoffConfig): string[] {
  ...
  if (config.features) {
    ...
    for (const [key, value] of Object.entries(features)) {
      if (OBJECT_FEATURES.includes(key as (typeof OBJECT_FEATURES)[number])) {
        ...
        if (!KNOWN_FEATURES.includes(key as (typeof KNOWN_FEATURES)[number])) {
          const moved = MOVED_TO_CACHING[key];
          if (moved) {
            errors.push(`Feature "${key}" moved to "${moved}" in schema v2 — it requires features.caching.enabled`);
          } else {
    ...
    const cachingRaw = features.caching as Record<string, unknown> | undefined;

    const connectivity = features.connectivity as Record<string, unknown> | undefined;
    if (connectivity && typeof connectivity === "object") {
      if (connectivity.enabled !== undefined && typeof connectivity.enabled !== "boolean") {
        errors.push("features.connectivity.enabled must be a boolean");
      }
      if (connectivity.heartbeatIntervalMs !== undefined && (typeof connectivity.heartbeatIntervalMs !== "number" || connectivity.heartbeatIntervalMs < 0 || !Number.isInteger(connectivity.heartbeatIntervalMs))) {
        errors.push("features.connectivity.heartbeatIntervalMs must be a non-negative integer");
      }
    }
    ...
    const caching = cachingRaw as Record<string, unknown> | undefined;
    if (caching && typeof caching === "object") {
      if (caching.enabled !== undefined && typeof caching.enabled !== "boolean") {
        errors.push("features.caching.enabled must be a boolean");
      }

      // Structural parent gate — no caching leaf may be effective without the umbrella on.
      const cachingOn = caching.enabled === true;
      const tiVal = caching.tagInvalidation as Record<string, unknown> | undefined;
      const mqVal = caching.mutationQueue as Record<string, unknown> | undefined;
      const gqlVal = caching.graphql as Record<string, unknown> | undefined;
      const spVal = caching.serverPush as Record<string, unknown> | undefined;
      if (!cachingOn) {
        if (tiVal && tiVal.enabled === true) {
          errors.push("features.caching.tagInvalidation.enabled requires features.caching.enabled to be true");
        }
        if (mqVal && (mqVal.enabled === true || mqVal.backgroundSync === true)) {
          errors.push("features.caching.mutationQueue requires features.caching.enabled to be true");
        }
        if (gqlVal && gqlVal.enabled === true) {
          errors.push("features.caching.graphql.enabled requires features.caching.enabled to be true");
        }
        if (spVal && spVal.enabled === true) {
          errors.push("features.caching.serverPush.enabled requires features.caching.enabled to be true");
        }
      }

      if (caching.requestBatchWindowMs !== undefined) {
        const value = caching.requestBatchWindowMs;
        if (typeof value !== "number" || value < 0 || !Number.isInteger(value)) {
          errors.push("features.caching.requestBatchWindowMs must be a non-negative integer");
        }
      }

      const precache = caching.precache as Record<string, unknown> | undefined;
      if (precache && typeof precache === "object") {
        if (precache.concurrency !== undefined && (typeof precache.concurrency !== "number" || precache.concurrency < 1 || !Number.isInteger(precache.concurrency))) {
          errors.push("features.caching.precache.concurrency must be a positive integer");
        }
      }

      const strategy = caching.strategy as Record<string, unknown> | undefined;
      if (strategy && typeof strategy === "object") {
        if (strategy.default !== undefined && !VALID_STRATEGIES.includes(strategy.default as (typeof VALID_STRATEGIES)[number])) {
          errors.push(
            `Invalid features.caching.strategy.default "${strategy.default}". Must be one of: ${VALID_STRATEGIES.join(", ")}`,
          );
        }
        ...
        if (patterns && typeof patterns === "object") {
          for (const [pattern, entry] of Object.entries(patterns)) {
            if (typeof pattern !== "string" || pattern.trim() === "") {
              errors.push("features.caching.strategy.patterns keys must be non-empty strings");
              continue;
            }
            const opens = (pattern.match(/\{/g) || []).length;
            const closes = (pattern.match(/\}/g) || []).length;
            if (opens !== closes) {
              errors.push(`features.caching.strategy.patterns["${pattern}"] has unbalanced braces`);
            }
              ...
              if (strat !== "reactive") {
                for (const field of REACTIVE_FIELDS) {
                  if (obj[field] !== undefined) {
                    errors.push(`features.caching.strategy.patterns["${pattern}"].${field} is only valid with "reactive" strategy`);
                  }
                }
              } else {
                if (obj.staleTime !== undefined && (typeof obj.staleTime !== "number" || obj.staleTime < 0)) {
                  errors.push(`features.caching.strategy.patterns["${pattern}"].staleTime must be a non-negative number`);
                }
                if (obj.refetchInterval !== undefined && (typeof obj.refetchInterval !== "number" || obj.refetchInterval < 0)) {
                  errors.push(`features.caching.strategy.patterns["${pattern}"].refetchInterval must be a non-negative number`);
                }
                if (obj.refetchOnReconnect !== undefined && typeof obj.refetchOnReconnect !== "boolean") {
                  errors.push(`features.caching.strategy.patterns["${pattern}"].refetchOnReconnect must be a boolean`);
                }
                if (obj.refetchOnFocus !== undefined && typeof obj.refetchOnFocus !== "boolean") {
                  errors.push(`features.caching.strategy.patterns["${pattern}"].refetchOnFocus must be a boolean`);
                }
              }
            } else {
              errors.push(`features.caching.strategy.patterns["${pattern}"] must be a string or an object`);
            }
          }
        }
        if (strategy.normalizeKey !== undefined && typeof strategy.normalizeKey !== "boolean") {
          errors.push("features.caching.strategy.normalizeKey must be a boolean");
        }
        if (strategy.maxRuntimeCacheAge !== undefined && (typeof strategy.maxRuntimeCacheAge !== "number" || strategy.maxRuntimeCacheAge < 0 || !Number.isInteger(strategy.maxRuntimeCacheAge))) {
          errors.push("features.caching.strategy.maxRuntimeCacheAge must be a non-negative integer");
        }
        if (strategy.ignoreQueryParams !== undefined && (!Array.isArray(strategy.ignoreQueryParams) || !(strategy.ignoreQueryParams as unknown[]).every((p: unknown) => typeof p === "string"))) {
          errors.push("features.caching.strategy.ignoreQueryParams must be an array of strings");
        }
        if (strategy.timeout !== undefined && (typeof strategy.timeout !== "number" || strategy.timeout <= 0)) {
          errors.push("features.caching.strategy.timeout must be a positive number");
        }
        ...
        if (reactive && typeof reactive === "object") {
          if (reactive.staleTime !== undefined && (typeof reactive.staleTime !== "number" || reactive.staleTime < 0)) {
            errors.push("features.caching.strategy.reactive.staleTime must be a non-negative number");
          }
          if (reactive.refetchInterval !== undefined && (typeof reactive.refetchInterval !== "number" || reactive.refetchInterval < 0 || !Number.isInteger(reactive.refetchInterval))) {
            errors.push("features.caching.strategy.reactive.refetchInterval must be a non-negative integer");
          }
          if (reactive.refetchOnReconnect !== undefined && typeof reactive.refetchOnReconnect !== "boolean") {
            errors.push("features.caching.strategy.reactive.refetchOnReconnect must be a boolean");
          }
          if (reactive.refetchOnFocus !== undefined && typeof reactive.refetchOnFocus !== "boolean") {
            errors.push("features.caching.strategy.reactive.refetchOnFocus must be a boolean");
          }
        }
      }
      const navigation = caching.navigation as Record<string, unknown> | undefined;
      if (navigation && typeof navigation === "object") {
        if (navigation.mode !== undefined && !["spa", "default", "ssr"].includes(navigation.mode as string)) {
          errors.push('features.caching.navigation.mode must be "spa", "default", or "ssr"');
        }
        if (navigation.fallback !== undefined && typeof navigation.fallback !== "string") {
          errors.push("features.caching.navigation.fallback must be a string");
        }
        if (navigation.preload !== undefined && typeof navigation.preload !== "boolean") {
          errors.push("features.caching.navigation.preload must be a boolean");
        }
        if (navigation.precacheRoutes !== undefined) {
          if (!Array.isArray(navigation.precacheRoutes) || !(navigation.precacheRoutes as unknown[]).every((r) => typeof r === "string")) {
            errors.push("features.caching.navigation.precacheRoutes must be an array of strings");
          }
        }
        const rules = navigation.rules as unknown[] | undefined;
        if (rules !== undefined) {
          if (!Array.isArray(rules)) {
            errors.push("features.caching.navigation.rules must be an array");
          } else {
            for (let i = 0; i < rules.length; i++) {
              const rule = rules[i] as Record<string, unknown>;
              if (!rule || typeof rule !== "object") {
                errors.push(`features.caching.navigation.rules[${i}] must be an object`);
                continue;
              }
              if (typeof rule.match !== "string" || rule.match.trim() === "") {
                errors.push(`features.caching.navigation.rules[${i}].match must be a non-empty string`);
              }
              if (rule.fallback !== undefined && typeof rule.fallback !== "string") {
                errors.push(`features.caching.navigation.rules[${i}].fallback must be a string`);
              }
            }
          }
        }
      ...
      const refetchQueue = caching.refetchQueue as Record<string, unknown> | undefined;
      if (refetchQueue && typeof refetchQueue === "object") {
        if (refetchQueue.batchSize !== undefined && (typeof refetchQueue.batchSize !== "number" || refetchQueue.batchSize < 1 || !Number.isInteger(refetchQueue.batchSize))) {
          errors.push("features.caching.refetchQueue.batchSize must be a positive integer");
        }
        if (refetchQueue.batchDelayMs !== undefined && (typeof refetchQueue.batchDelayMs !== "number" || refetchQueue.batchDelayMs < 0 || !Number.isInteger(refetchQueue.batchDelayMs))) {
          errors.push("features.caching.refetchQueue.batchDelayMs must be a non-negative integer");
        }
        const retry = refetchQueue.retry as Record<string, unknown> | undefined;
        if (retry !== undefined) {
          if (typeof retry !== "object" || retry === null) {
            errors.push("features.caching.refetchQueue.retry must be an object");
          } else {
            if (retry.maxRetries !== undefined && (typeof retry.maxRetries !== "number" || retry.maxRetries < 0 || !Number.isInteger(retry.maxRetries))) {
              errors.push("features.caching.refetchQueue.retry.maxRetries must be a non-negative integer");
            }
            if (retry.backoffMs !== undefined && (typeof retry.backoffMs !== "number" || retry.backoffMs < 0 || !Number.isInteger(retry.backoffMs))) {
              errors.push("features.caching.refetchQueue.retry.backoffMs must be a non-negative integer");
            }
            if (retry.maxBackoffMs !== undefined && (typeof retry.maxBackoffMs !== "number" || retry.maxBackoffMs < 0 || !Number.isInteger(retry.maxBackoffMs))) {
              errors.push("features.caching.refetchQueue.retry.maxBackoffMs must be a non-negative integer");
            }
            if (retry.jitterMs !== undefined && (typeof retry.jitterMs !== "number" || retry.jitterMs < 0 || !Number.isInteger(retry.jitterMs))) {
              errors.push("features.caching.refetchQueue.retry.jitterMs must be a non-negative integer");
            }
          }
        }
      }

      if (tiVal && typeof tiVal === "object") {
        const ti = tiVal;
        if (ti.debounceMs !== undefined && (typeof ti.debounceMs !== "number" || ti.debounceMs < 0 || !Number.isInteger(ti.debounceMs))) {
          errors.push("features.caching.tagInvalidation.debounceMs must be a non-negative integer");
        }
        if (ti.skipPrefixes !== undefined && (!Array.isArray(ti.skipPrefixes) || !(ti.skipPrefixes as unknown[]).every((p) => typeof p === "string"))) {
          errors.push("features.caching.tagInvalidation.skipPrefixes must be an array of strings");
        }
        if (ti.patterns !== undefined && (typeof ti.patterns !== "object" || ti.patterns === null)) {
          errors.push("features.caching.tagInvalidation.patterns must be an object");
        }
        if (ti.singularization !== undefined && (typeof ti.singularization !== "object" || ti.singularization === null)) {
          errors.push("features.caching.tagInvalidation.singularization must be an object");
        }
        if (ti.cascading !== undefined && (typeof ti.cascading !== "object" || ti.cascading === null)) {
          errors.push("features.caching.tagInvalidation.cascading must be an object");
        }
      }

      if (mqVal && typeof mqVal === "object") {
        const mq = mqVal;
        if (mq.enabled !== undefined && typeof mq.enabled !== "boolean") {
          errors.push("features.caching.mutationQueue.enabled must be a boolean");
        }
        if (mq.batchSize !== undefined && (typeof mq.batchSize !== "number" || mq.batchSize < 1 || !Number.isInteger(mq.batchSize))) {
          errors.push("features.caching.mutationQueue.batchSize must be a positive integer");
        }
        if (mq.batchDelayMs !== undefined && (typeof mq.batchDelayMs !== "number" || mq.batchDelayMs < 0 || !Number.isInteger(mq.batchDelayMs))) {
          errors.push("features.caching.mutationQueue.batchDelayMs must be a non-negative integer");
        }
        const mqRetry = mq.retry as Record<string, unknown> | undefined;
        if (mqRetry !== undefined) {
          if (typeof mqRetry !== "object" || mqRetry === null) {
            errors.push("features.caching.mutationQueue.retry must be an object");
          } else {
            if (mqRetry.maxRetries !== undefined && (typeof mqRetry.maxRetries !== "number" || mqRetry.maxRetries < 0 || !Number.isInteger(mqRetry.maxRetries))) {
              errors.push("features.caching.mutationQueue.retry.maxRetries must be a non-negative integer");
            }
            if (mqRetry.backoffMs !== undefined && (typeof mqRetry.backoffMs !== "number" || mqRetry.backoffMs < 0 || !Number.isInteger(mqRetry.backoffMs))) {
              errors.push("features.caching.mutationQueue.retry.backoffMs must be a non-negative integer");
            }
            if (mqRetry.maxBackoffMs !== undefined && (typeof mqRetry.maxBackoffMs !== "number" || mqRetry.maxBackoffMs < 0 || !Number.isInteger(mqRetry.maxBackoffMs))) {
              errors.push("features.caching.mutationQueue.retry.maxBackoffMs must be a non-negative integer");
            }
            if (mqRetry.jitterMs !== undefined && (typeof mqRetry.jitterMs !== "number" || mqRetry.jitterMs < 0 || !Number.isInteger(mqRetry.jitterMs))) {
              errors.push("features.caching.mutationQueue.retry.jitterMs must be a non-negative integer");
            }
          }
        }
        if (mq.backgroundSync !== undefined && typeof mq.backgroundSync !== "boolean") {
          errors.push("features.caching.mutationQueue.backgroundSync must be a boolean");
        }
        if (mq.backgroundSync === true && mq.enabled !== true) {
          errors.push("features.caching.mutationQueue.backgroundSync requires mutationQueue to be enabled");
        }
      }
      ...
      if (gqlVal && typeof gqlVal === "object") {
        const graphql = gqlVal;
        if (graphql.enabled !== undefined && typeof graphql.enabled !== "boolean") {
          errors.push("features.caching.graphql.enabled must be a boolean");
        }
        if (graphql.endpoints !== undefined) {
          if (!Array.isArray(graphql.endpoints) || !(graphql.endpoints as unknown[]).every((e) => typeof e === "string")) {
            errors.push("features.caching.graphql.endpoints must be an array of strings");
          }
        }
      }
      ...
      if (spVal && typeof spVal === "object") {
        ...
        if (sp.enabled !== undefined && typeof sp.enabled !== "boolean") {
          errors.push("features.caching.serverPush.enabled must be a boolean");
        }
        if (sp.enabled && features.auth && typeof features.auth === "object" && (features.auth as Record<string, unknown>).enabled && (features.auth as Record<string, unknown>).type === "bearer") {
          errors.push("features.caching.serverPush is not supported with bearer auth — use cookie auth instead");
        }
        if (sp.enabled) {
          const ti = tiVal;
          if (cachingOn && (!ti || ti.enabled !== true)) {
            errors.push("features.caching.serverPush.enabled requires features.caching.tagInvalidation.enabled to be true");
          }
        }
        if (sp.type !== undefined && !["sse", "websocket"].includes(sp.type as string)) {
          errors.push('features.caching.serverPush.type must be "sse" or "websocket"');
        }
        if (sp.endpoint !== undefined && typeof sp.endpoint !== "string") {
          errors.push("features.caching.serverPush.endpoint must be a string");
        }
        if (sp.reconnectDelayMs !== undefined && (typeof sp.reconnectDelayMs !== "number" || sp.reconnectDelayMs < 0 || !Number.isInteger(sp.reconnectDelayMs))) {
          errors.push("features.caching.serverPush.reconnectDelayMs must be a non-negative integer (capped at 30000ms)");
        }
        if (sp.reconnectDelayMs !== undefined && typeof sp.reconnectDelayMs === "number" && sp.reconnectDelayMs > 30000) {
          errors.push("features.caching.serverPush.reconnectDelayMs is capped at 30000ms (30s)");
        }
      }
  ...
  const VALID_FRAMEWORKS = ["nextjs", "remix", "tanstack-start-react", "astro", "nuxt", "vike", "sveltekit", "react", "vue", "svelte", "qwik", "preact", "angular", "solid", "lit", "alpine", "marko", "stimulus", "jquery", "htmx", "vanilla", "no-bundler"];
  if (fw !== undefined) {
    if (typeof fw !== "string") {
