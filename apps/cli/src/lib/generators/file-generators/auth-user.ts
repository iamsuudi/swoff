...
export function generateAuthUser(ctx: GeneratorContext): void {
  ...
  const cookieAuth = type === "cookie";
  const authTypeName = type === "cookie" ? "cookie" : "bearer";
 ...
 */
export async function refreshSession(${cookieAuth ? "" : "token"+T("string")})${R("Promise<Response>")}{
  ${cookieAuth
    ? `return fetch(API_BASE + "${refreshPath}", {
    method: "POST",
    credentials: "include" as RequestCredentials,
  });`
    : `const headers${T("Record<string, string>")} = { "Content-Type": "application/json" };
