...
`
  ...
  const authHeader${T(ts, "Record<string, string>")} = auth?.token ? { Authorization: \`Bearer \${auth.token}\` } : {};
`
    : "";
...
export async function queueMutation(mutation${T(ts, "Partial<MutationQueueItem>")})${R(ts, "Promise<void>")}{
  ...
  const safeHeaders${T(ts, "Record<string, string>")} = { ...(mutation.headers || {}) };
...
async function replayMutation(item${T(ts, "MutationQueueItem")}, db${T(ts, "IDBDatabase | undefined")})${R(ts, "Promise<boolean>")}{
  try {
${authReplayHeaders}    let replayBody${T(ts, "BodyInit | null")}${ts ? " = null" : ""};
    let contentType${T(ts, "string | undefined")};
    const bt = item.bodyType || "json";
    if (bt === "formdata") {
      replayBody = new FormData();
      for (const [key, value] of (item.body || [])${AS(ts, "[string, FormDataEntryValue][]")}) {
        replayBody.append(key, value);
      }
    } else if (bt === "blob") {
      replayBody = item.body${AS(ts, "BodyInit | null")};
    } else if (bt === "buffer") {
      replayBody = item.body instanceof ArrayBuffer ? new Uint8Array(item.body)${AS(ts, "BodyInit")} : item.body${AS(ts, "BodyInit")};
    } else if (bt === "text") {
      replayBody = item.body${AS(ts, "BodyInit | null")};
...
export async function processMutationQueue()${R(ts, "Promise<void>")}{
  ...
  try {
    ...
    const queue${T(ts, "MutationQueueItem[]")} = await new Promise${PT(ts, "MutationQueueItem[]")}((resolve, reject) => {
...
export async function flushMutations()${R(ts, "Promise<void>")}{
...
export async function getPendingCount()${R(ts, "Promise<number>")}{
  const db = await openQueueDB();
  return new Promise${PT(ts, "number")}((resolve, reject) => {
    const tx = db.transaction(STORE_NAME, "readonly");
    const request = tx.objectStore(STORE_NAME).count();
    request.onsuccess = () => resolve((request${AS(ts, "IDBRequest<number>")}).result);
...
export async function getQueuePosition(id${T(ts, "string")})${R(ts, "Promise<number>")}{
  const items = await getQueueItems();
  return items.findIndex((item) => item.id === id);
}

/** Get all pending queue items with their details. */
export async function getQueueItems()${R(ts, "Promise<MutationQueueItem[]>")}{
  const db = await openQueueDB();
  const tx = db.transaction(STORE_NAME, "readonly");
  const store = tx.objectStore(STORE_NAME);
  const index = store.index("by-timestamp");
  return new Promise${PT(ts, "MutationQueueItem[]")}((resolve, reject) => {
    const request = index.getAll();
    request.onsuccess = () => resolve(request.result);
    request.onerror = () => reject(request.error);
  });
