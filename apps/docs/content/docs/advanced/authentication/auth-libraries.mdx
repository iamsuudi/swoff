...
Identify your auth library's mechanism to know whether you need to configure `withAuthHeaders()` in [Auth-Aware Fetch](/docs/advanced/authentication/auth-fetch) or can skip it entirely.
...
You can skip `withAuthHeaders()` entirely — return headers unchanged. The browser handles everything.

### Bearer Token Auth (Manual)

The server returns a **token string** on login. The client must store it and manually set `Authorization: Bearer <token>` on every request. **Header setup required.**

You must edit `withAuthHeaders()` to inject the token.

### Custom Header Auth (Manual)

Same as Bearer, but with a non-standard header name (e.g., `X-Auth-Token`, `X-API-Key`, `Authorization: Token`). **Header setup required.**

You must edit `withAuthHeaders()` to inject the token with your custom header.
...
- **Configure your fetch**: [Auth-Aware Fetch](/docs/advanced/authentication/auth-fetch)
