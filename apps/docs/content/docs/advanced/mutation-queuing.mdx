...
For apps that need to sync even after the user closes the tab, combine with [Background Sync](/docs/advanced/background-sync):
...
- [ ] If using cookie auth, include CSRF tokens in queued mutation `headers` (see [CSRF Considerations](#csrf-considerations))
...
3. **Idempotency keys** — to prevent duplicate processing when a mutation syncs but the response is lost, include an idempotency key in the mutation body. Your backend should reject duplicate keys.
...
- See [Background Sync](/docs/advanced/background-sync) — sync after tab close
- Resolve multi-user conflicts: [Conflict Resolution](/docs/advanced/conflict-resolution)
