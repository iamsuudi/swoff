...
**Your backend sends the push message** — Swoff covers the client-side subscription management and SW handlers. You build the backend part with your stack (Go, Node.js, Python, etc.).
...
Push notifications require a backend. If you just want to show notifications from client code (no server needed), use the Notifications API directly:
...
- [x] Local notification helper (no backend needed)
