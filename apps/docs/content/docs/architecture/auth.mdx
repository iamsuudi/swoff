...
flowchart TB
    CA["clearAuth()"] --> NULL["null memoryAuth"]
    NULL --> DELETE["delete 'session' from IndexedDB (swoff-auth DB)"]
    DELETE --> CACHE["delete runtime caches by prefix (swoff-runtime*)"]
    CACHE --> QUEUE["clear mutation queue (if mutationQueue enabled)"]
    QUEUE --> DISPATCH["dispatch sw-auth-state-change event"]
    DISPATCH --> POST["postMessage({ type: 'AUTH_CLEARED' }) to SW"]
    POST --> FORWARD["SW forwards to all clients via clients.matchAll()"]
    FORWARD --> CLIENT["each client calls clearMemoryAuth()"]
```
...
> See the [Library Comparison](/docs/comparisons#tier-1-sw-toolkits) for auth module comparison against Workbox, TanStack Query, and client databases.
