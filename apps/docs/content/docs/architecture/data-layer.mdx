...
**Invalidate:** Delete all cached responses tagged with related tags (e.g., after POST `/api/todos`, invalidate all cached queries tagged `todos`). The next read fetches fresh data. The SW also broadcasts `TAG_INVALIDATED` to all open tabs so they refetch automatically (see [Cross-Tab Sync](/docs/guides/data-fetching)).
...
Setup requires calling `initCrossTabSync()` once after SW registration (see [API Integration](/docs/guides/data-fetching)).
...
1. [Offline-First Architecture](/docs/architecture/offline-architecture) — understand the client runtime
2. [SW Template](/docs/cli/) — foundation of the caching system
3. [API Integration](/docs/guides/data-fetching) — fetch wrapper with cache strategy

## Next Steps

- [API Integration](/docs/guides/data-fetching) — tag-aware fetch wrapper
- [Mutation Queuing](/docs/guides/offline-mutations) — queue, reconcile, invalidate
