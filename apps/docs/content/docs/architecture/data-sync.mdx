...
- `useSwoffQueue()` — exposes `{ pending, items, lastSync }`
- `useSwoffMutationState(id)` — subscribes to a specific mutation's state changes
...
> See the [Library Comparison](/docs/comparisons#tier-1-sw-toolkits) for cross-tab sync and request batching comparisons against TanStack Query, Apollo Client, RxDB, and Workbox.
