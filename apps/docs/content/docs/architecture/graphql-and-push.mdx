...
flowchart LR
    INVAL["On tag invalidation"] --> DB["open tag DB → find all entries matching tag"]
    DB --> DELETE["delete cache entries from runtime cache (by cache key URL)"]
    DELETE --> QUEUE["queueRefresh(entry.actualUrl) for each (deduplicated via batch queue)"]
    QUEUE --> NOTIFY["on successful refetch → CACHE_UPDATED to all clients"]
```
