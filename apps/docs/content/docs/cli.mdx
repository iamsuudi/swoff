...
No source image? A wordmark icon is generated from `--app-name`. Run `npx @swoff/assets --help` for all flags (`--source`, `--no-splash`, `--theme-color`, `--orientation`, `--print-schema`, etc.).
...
| `public/mipmap-*/ic_launcher*.png`    | Android adaptive icon stack (5 densities + foreground/monochrome, always-on) |
...
36 files by default, up to 50 with `--monochrome`, `--ms-tile-color`, and dark mode.
