...
Swoff operates at the **browser infrastructure layer** — the Service Worker + `fetch` event — not the application layer. This means it works with any backend (PHP, Laravel, Django, Rails, Go, Java), any frontend (React, Vue, Svelte, HTMX, vanilla), and any rendering strategy (SSR, SSG, SPA, islands, HTML-over-wire). See [Ecosystem Compatibility](/docs/cli/ecosystem).

> **Integration guides:** See [guides](/docs/frameworks) for step-by-step walkthroughs — start with PWA, add data fetching, auth, tag invalidation, GraphQL, offline mutations, push, and server push incrementally.
...
- [Offline Navigation](/docs/comparisons/offline-navigation) — 3 navigation modes, HTML cache isolation, auto-prefetch, per-route fallback rules vs Serwist / next-pwa / Workbox
- [Data Fetching](/docs/comparisons/data-fetching) — `useCachedFetch` vs TanStack Query / SWR
- [Caching & Data Layer](/docs/comparisons/caching-and-data) — SW-level HTTP caching vs client databases
- [Cache Invalidation](/docs/comparisons/invalidation) — Tag-based vs query-key
- [Request Batching](/docs/comparisons/request-batching) — 50ms coalescing (unique to Swoff)
- [Offline Queue](/docs/comparisons/offline-queue) — Mutation queue vs TanStack Query / Workbox / client DBs
- [Service Worker](/docs/comparisons/service-worker) — Generated auditable SW vs Workbox runtime
- [GraphQL](/docs/comparisons/graphql) — Body-hash caching vs Apollo/Relay normalized cache
- [Auth](/docs/comparisons/auth) — SW-level auth vs manual injection
- [Real-time](/docs/comparisons/realtime) — SSE push vs WebSocket subscriptions vs polling
- [Cross-tab Sync](/docs/comparisons/cross-tab) — SW broadcast vs BroadcastChannel vs storage events
- [PWA](/docs/comparisons/pwa) — Manifest gen + install prompt + push notifications vs vite-plugin-pwa
- [Client DBs](/docs/comparisons/caching-and-data#the-client-db-approach-and-its-costs) — RxDB / TanStack DB / ElectricSQL / PowerSync (covered in Caching & Data Layer)
- [Optimistic Updates](/docs/comparisons/optimistic-updates) — Why Swoff excludes them (design rationale)
- [Developer Experience](/docs/comparisons/devx) — Config-driven code gen vs runtime library setup
