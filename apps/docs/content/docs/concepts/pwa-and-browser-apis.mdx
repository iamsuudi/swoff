...
A JSON file that tells the browser the app is installable. It specifies the app name, icons, start URL, and display mode (standalone vs browser tab).

### 2. Install Prompt

The browser fires a `beforeinstallprompt` event on supported platforms. Your app can capture this to show a custom "Install App" button. On platforms that don't support it (iOS Safari), users add to the home screen via the Share menu.
...
Store and retrieve network requests/responses. Used by the Service Worker to cache API responses and serve them when offline.

### IndexedDB

Client-side database for structured data. Used for the mutation queue (pending writes) and local application data.

### Fetch API

Make network requests. Swoff wraps this with caching, tagging, and invalidation logic.
...
- [Offline-First Architecture](/docs/architecture/offline-architecture) — how to build with these APIs
- [iOS Safari Limitations](/docs/concepts/ios-safari-limitations) — platform constraints
