...
| Field                   | Type      | Default | Description                                                                                    |
| ----------------------- | --------- | ------- | ---------------------------------------------------------------------------------------------- |
| `enabled`               | `boolean` | `true`  | Enable PWA installability (generates `pwa/prompt.{ts\|js}`)                                    |
...
| Field          | Type                                              | Default     | Description                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------- | ------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `version`      | `"package"` \| `"hash"` \| `"manual"` \| `string` | `"package"` | SW version mode. Generates `swoff/sw/version.{ts\|js}` as the single version source. `"package"`: auto-reads from `package.json` at build time (requires `package.json`). `"hash"`: every build produces a unique cache name (uses `Date.now().toString(36)`), SW URL stays fixed — no `package.json` needed. `"manual"`: user edits `swoff/sw/version.{ts\|js}` directly. Any string works — no semver or keyword validation. |
...
| `type`       | `"bearer"` \| `"cookie"` \| `"custom"` | `"bearer"`                                                                                                    | Auth adapter type. Determines how Swoff communicates with your auth provider. See [auth/adapter.ts](/docs/api/auth) in API Reference.                                                                             |
