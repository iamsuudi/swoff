...
| `navCacheOnly` | `boolean` | `false` | When `true`, only intercept and cache navigation requests. Subresource requests (API, JS, CSS, images) pass through to the network — letting in-memory caching libraries (TanStack Query, SWR, Apollo Client) manage data freshness independently. |
