...
| `clearRuntimeOnUpdate` | boolean | `false` | Clear runtime cache on SW activate (security measure) |
| `navigationMode` | string | `"spa"` | SPA navigation fallback or default strategy |
| `spaEntry` | string | `"/index.html"` | Fallback URL for SPA navigation mode |
...
| `runtimeCacheName` | `"swoff-runtime"` | Runtime cache storage name |
