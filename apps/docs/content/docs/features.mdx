...
<Cards>
  ...
  >
    Queues offline writes in IndexedDB, auto-syncs on reconnect, reconciles temp
    IDs.
  </Card>
  <Card
  ...
  >
    Token storage, auth-aware fetch, current user cache, SW integration, offline
    state, lock screen.
  </Card>
  <Card
    title="Testing"
  ...
  >
    DevTools walkthrough, postMessage monitoring, and troubleshooting tables for
...
<Cards>
  ...
  <Card title="Add more retries" icon={<RotateCwIcon />}>
    In <code>mutation-queue.js</code>, bump <code>MAX_RETRIES</code> from 5 to
    10, or implement exponential backoff.
  </Card>
