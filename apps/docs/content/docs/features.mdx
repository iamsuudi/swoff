...
| [TypeScript Utilities](/docs/patterns/utilities)        | Type declarations for SW window properties, `checkForUpdate()` helper                      | Type definitions, helper function behavior                                                            |
...
Every Swoff feature is designed to be modified. Here's what that looks like in practice:

- **Change a caching strategy**: In `sw-template.js`, modify the `fetch` handler to use `NetworkFirst` for API routes instead of `CacheFirst`
- **Swap IndexedDB wrappers**: Start with native IndexedDB, later switch to the `idb` library — only the DB utility functions change, not your app logic
- **Custom merge logic**: In `conflict.js`, write a per-field merge function — e.g., `title: serverWins, completed: localOrServer, tags: union, comments: append`
- **Replace the update prompt**: The `sw-update-available` event lets you build any UI you want — toast, modal, banner, or auto-update
- **Add more retries**: In `mutation-queue.js`, bump `MAX_RETRIES` from 5 to 10, or implement exponential backoff
- **Scope auth to cookie-based**: Edit `withAuthHeaders()` in `auth-fetch.js` to be a no-op when using cookie/session auth

## Next Steps

- Start with the [Introduction](/docs/introduction) if you haven't already
- Learn the [concepts](/docs/concepts/what-is-offline) — offline capability and service workers
