...
| -------------------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------- |
| **Laravel / PHP**          | Place `swoff/` under `public/`, reference in Blade template                                    | [Laravel guide](/docs/framework-adapters/laravel) |
| **Django**                 | Place under `static/`, use `{% static %}` tag, or collect via `python manage.py collectstatic` | [Django & Flask guide](/docs/framework-adapters/django)   |
| **Flask**                  | Place under `static/`, reference via `url_for('static', ...)`                                   | [Django & Flask guide](/docs/framework-adapters/django)   |
| **Rails**                  | Place under `app/assets/` or `public/` with asset pipeline                                     | [Rails guide](/docs/framework-adapters/rails)     |
| **Go / net/http**          | Embed via `//go:embed swoff/*` or serve from a `static/` directory                             | [Go guide](/docs/framework-adapters/go)           |
| **HTMX**                   | Include `<script>` tag, control via `hx-headers`                                               | [HTMX guide](/docs/framework-adapters/htmx)       |
| **Plain HTML / Static**    | Serve from document root and reference with a `<script>` tag                                   | [Vanilla guide](/docs/framework-adapters/vanilla) |
