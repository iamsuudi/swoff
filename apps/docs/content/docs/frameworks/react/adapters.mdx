...
> **Astro with React islands:** See the [dedicated Astro page](/docs/frameworks/astro/adapters) if you're using Astro (with or without React). The React hooks are still available for `client:load` React components.
...
> For detailed setup guides, see [React SPA](/docs/frameworks/react/spa), [TanStack Start](/docs/frameworks/react/tanstack-start), and [Next.js](/docs/frameworks/react/nextjs).
...
| `usePrecacheProgress` | always                         | Background precaching progress               |
...
`status` is `"installing"` during background precaching, `"idle"` otherwise. Returns `progress` (0–100) while installing.
...
See the [Next.js guide](/docs/frameworks/react/nextjs) for App Router and Pages Router setup.
