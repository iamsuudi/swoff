...
| Setting                                | Value           | Why                                                                                      |
| -------------------------------------- | --------------- | ---------------------------------------------------------------------------------------- |
| `caching.strategy.ignoreQueryParams`   | `["_rsc"]`      | Strips `_rsc` from cache keys. `GET /page` and `GET /page?_rsc=1` map to the same entry. |
| `pattern./_next/static*`               | `cache-first`   | Hashed filenames — once cached, never stale.                                             |
| `caching.strategy.default`             | `network-first` | Dynamic SSR pages try the network first, fall back to cached HTML.                       |
