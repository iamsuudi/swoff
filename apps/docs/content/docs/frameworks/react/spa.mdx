...
React SPAs often have API routes (`/api/*`) for data fetching. These use `network-first` — they work offline only if visited before and cached in runtime. Mutations (POST/PUT/DELETE) always need the network — see [Offline Mutations](/docs/guides#mutation-queue) for queueing.
