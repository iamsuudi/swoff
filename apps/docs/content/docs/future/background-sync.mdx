...
**Fallback:** For unsupported browsers, the mutation queue auto-processes on the `online` event (already set up in [Mutation Queuing](/docs/future/mutation-queuing)). The fallback only works while the tab is open.

## Prerequisites

1. [Mutation Queuing](/docs/future/mutation-queuing) — the queue must be set up first
...
- See [Mutation Queuing](/docs/future/mutation-queuing) — the queue that powers the sync
