...
async function createTodo(title) {
  ...
  if (navigator.onLine) {
    // Send directly — SW lets mutations pass through
    ...
    // Queue for later — sync, reconcile, and invalidate will run when online
    await queueMutation({
      method: "POST",
      url: "/api/todos",
      body: todo,
...
export function useMutationQueue() {
  ...
  return { pendingCount, syncNow: processMutationQueue };
}
```
...
- [x] `queueMutation()` with full metadata
- [x] `processMutationQueue()` with fetch, reconcile, invalidate
