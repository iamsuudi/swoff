...
> `strategy.reactive` is present when `strategy.default` is `"reactive"`. For any other default strategy the block is omitted from generated configs (it only carries defaults for the reactive strategy).
...
- [Library Comparison](/docs/comparisons#tier-1-sw-toolkits) — Swoff vs Workbox / Serwist / vite-plugin-pwa / next-pwa
