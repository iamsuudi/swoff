---
title: GraphQL
description: Body-hash caching, operation-name auto-tags, and React hooks for GraphQL.
---
...
GraphQL uses the same `useCachedFetch` adapter pattern described in the [Data Fetching guide](/docs/guides/data-fetching#framework-adapters). When a mutation auto-invalidates by operation name, the SW dispatches `cache-invalidated`:
...
```js
// Same pattern as useCachedFetch:
...
No GraphQL-specific adapter is needed — the same `useCachedFetch` pattern handles both REST and GraphQL. Body-hash caching and operation-name auto-tags happen automatically in the SW.

**If your framework already has this** (React): the CLI generates `useCachedFetch` into `swoff/adapters/`. See the [React page](/docs/frameworks/react/adapters) or the [Blueprint](/docs/frameworks/blueprint).
