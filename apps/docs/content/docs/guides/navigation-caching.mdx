...
Three modes control offline navigation behavior. For the internal `serveFromCache` lookup rules, fallback chain depth, and SSR auto-prefetch internals, see the [Caching System architecture](/docs/architecture/caching#navigation-modes).
...
- `navigation.mode` — `"spa"`, `"ssr"`, or `"default"`. See the [architecture page](/docs/architecture/caching#navigation-modes) for per-mode lookup and fallback details.
...
- [Comparison: SW Toolkits](/docs/comparisons#tier-1-sw-toolkits) — offline navigation feature matrix
