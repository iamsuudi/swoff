...
Set the mutation queue (and optionally Background Sync) in `swoff.config.json`, then regenerate:
...
?> Background Sync requires cookie auth and is not supported in Firefox or Safari.
