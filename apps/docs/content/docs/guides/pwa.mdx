...
The SW tracks which routes fell back to cached or offline responses. The `useSwoffAnalytics` adapter exposes `{ lastEvent, events, clear }` — see the [Navigation Caching guide](/docs/guides/navigation-caching#useswoffanalytics) for usage and event shapes.
