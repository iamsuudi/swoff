...
> **If you're coming from Workbox:** Swoff generates a complete, auditable service worker from config — no manual SW code, no runtime Workbox modules bundled into your app. The generated SW is a `.js` file you can read, debug, and commit. See the [Comparison: SW Toolkits](/docs/comparisons#tier-1-sw-toolkits).
...
If your framework already has a `useSwoffPwa` adapter (React, Vue, Svelte), the CLI generates it into `swoff/adapters/`. See the [Blueprint](/docs/frameworks/blueprint) for the event reference and adapter patterns.

`useSwoffPrecache` is covered in the [Service Worker guide](/docs/guides/service-worker#framework-adapters). `useSwoffStorage` is covered in the [Connectivity guide](/docs/guides/connectivity).

## Related

- [Comparison: SW Toolkits](/docs/comparisons#tier-1-sw-toolkits) — full PWA feature matrix
