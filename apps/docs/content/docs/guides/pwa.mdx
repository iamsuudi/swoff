...
`useStorageEstimate` now lives in the [Connectivity & Storage guide](/docs/guides/connectivity#usestorageestimate).

**If your framework already has these adapters** (React): the CLI generates `usePwaInstall` and `useSWUpdate` into `swoff/adapters/`. See the [React page](/docs/frameworks/react/adapters) for their APIs. To create adapters for any other framework, see the [Blueprint](/docs/frameworks/blueprint).
