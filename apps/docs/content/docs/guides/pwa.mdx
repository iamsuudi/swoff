...
<script>
  ...
  // Storage estimate
  const { usage, quota, percentUsed } = await swoff.getStorageEstimate();

  // Start connectivity heartbeat (required for useNetworkStatus adapters)
  swoff.startHeartbeat();
</script>
```
...
See the [Service Worker guide](/docs/guides/service-worker#framework-adapters) for the adapter that exposes `{ updateStatus, progress, error, acceptUpdate, dismissUpdate }` from SW lifecycle events.

### `useStorageEstimate`

Exposes storage stats: `{ usage, quota, percentUsed }`.

**Events listened to:** None. Calls `navigator.storage.estimate()` directly.

**Building the adapter:**

```js
// Reactive state: { usage, quota, percentUsed }
// 1. On mount, call navigator.storage.estimate()
// 2. usage = result.usage, quota = result.quota
// 3. percentUsed = (usage / quota) * 100
// 4. Re-read on focus + visibilitychange for freshness
// 5. Return { usage, quota, percentUsed }
```

**If your framework already has these adapters** (React): the CLI generates `usePwaInstall`, `useSWUpdate`, and `useStorageEstimate` into `swoff/adapters/`. See the [React page](/docs/frameworks/react) for their APIs. To create adapters for any other framework, see the [Blueprint](/docs/frameworks/blueprint).
