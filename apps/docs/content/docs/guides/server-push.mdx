...
The adapter pattern is identical to the one described in the [Tag Invalidation guide](/docs/guides/tag-invalidation#frameworks) — the only difference is the source of invalidation (server push instead of manual calls). See the [Blueprint](/docs/frameworks/blueprint) for the event reference and adapter patterns.
