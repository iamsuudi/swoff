...
```js
// Reactive state shape: { status, progress }
// 1. Initialize: status = "idle", progress = 0
// 2. Listen to events:
window.addEventListener("sw-progress", (e) => {
  // Set status = "installing", progress = e.detail.percent
});
// 3. Return { status, progress }
```

**If your framework already has this adapter** (React): the CLI generates `usePrecacheProgress` into `swoff/adapters/`. See the [React page](/docs/frameworks/react/adapters) for its API. To create adapters for any other framework, see the [Blueprint](/docs/frameworks/blueprint).
