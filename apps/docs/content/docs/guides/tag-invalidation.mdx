...
**If your framework already has this** (React): the CLI generates `useCachedFetch` into `swoff/adapters/` which listens to `cache-invalidated` automatically. See the [React page](/docs/frameworks/react) or the [Blueprint](/docs/frameworks/blueprint).
