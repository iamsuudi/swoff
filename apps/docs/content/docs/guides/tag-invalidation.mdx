---
title: Tag System
description: Auto-tags, glob patterns, cascading invalidation, cross-tab, and framework adapters.
---

> **If you're coming from TanStack Query:** `invalidateByTag("notes")` is Swoff's equivalent of `queryClient.invalidateQueries({ queryKey: ["notes"] })`. The difference: Swoff auto-generates tags from URL patterns, supports glob matching, cascading invalidation, and works cross-tab — no query key management needed. See the [GraphQL & Push architecture](/docs/architecture/graphql-and-push).
...
flowchart TB
    A["Tab A: calls invalidateByTag('notes')"] --> POST["postMessage to SW"]
    POST --> INVAL["SW invalidates cache entries tagged 'notes'"]
    INVAL --> BROADCAST["SW broadcasts 'cache-invalidated' to all tabs (A, B, C, ...)"]
    BROADCAST --> B["Tab B's next fetchWithCache('/api/notes') returns fresh data"]
```
...
The `cache-invalidated` event drives the `useSwoffFetch` adapter (see the [Data Fetching guide](/docs/guides/data-fetching#framework-adapters)). When tags are invalidated, your adapter must listen for this event and refetch affected data.
...
If your framework already has a `useSwoffFetch` adapter (React, Vue, Svelte), the CLI generates it into `swoff/adapters/` — it listens to `cache-invalidated` automatically. To create adapters for any other framework, see the [Blueprint](/docs/frameworks/blueprint) for the event reference and adapter patterns.
...
- [Library Comparison](/docs/comparisons) — cross-tab sync in Tier 1 feature matrix
- [Optimistic updates design rationale](/docs/architecture/optimistic-updates)
