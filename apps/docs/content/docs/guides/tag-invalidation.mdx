...
**If your framework already has this** (React): the CLI generates `useSwoffFetch` into `swoff/adapters/` which listens to `cache-invalidated` automatically. See the [React page](/docs/frameworks/react/adapters) or the [Blueprint](/docs/frameworks/blueprint).
