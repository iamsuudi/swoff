...
function NoteDetailPage() {
  ...
  const {
    ...
    loading,
    error,
  } = useCachedFetch<Note>(`/api/notes/${id}`, { auth: true })
  ...
  const handleDelete = async () => {
    ...
    if (result.status === 'error')
      setDeleteError(result.error.message || 'Failed to delete note')
  }
      ...
      <div className="flex min-h-screen items-center justify-center bg-gray-50 dark:bg-gray-900">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            {error?.message || 'This note may have been deleted'}
          </p>
          <Link
            to="/"
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-red-500 transition hover:bg-red-50 dark:border-gray-600 dark:bg-gray-800 dark:hover:bg-red-900/30 disabled:opacity-50"
            >
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8 dark:border-gray-700 dark:bg-gray-800">
          <div className="mb-6 flex flex-wrap items-center gap-3">
            <h1 className="flex-1 text-2xl font-bold text-gray-900 sm:text-3xl dark:text-white">
              {note.title}
            </h1>
            <span
              className={`rounded-full border px-3 py-1 text-xs font-medium ${priorityBadgeColors[note.priority] || priorityBadgeColors.low}`}
            >
              {note.priority}
            </span>
          </div>
          <p className="whitespace-pre-wrap text-gray-600 leading-relaxed dark:text-gray-400">
            {note.description}
          </p>
