...
function filterNotesByQuery(notes: Note[], q: string): Note[] {
  if (!q) return notes
...
async function NotesList({ q }: { q: string }) {
  const notes = (await getNotes()) as Note[]
  const filtered = filterNotesByQuery(notes, q)
...
const getNotesRSC = createServerFn()
  .validator((data: { q: string }) => data)
  .handler(async ({ data }) => {
    const Renderable = await renderServerComponent(<NotesList q={data.q} />)
    return { notes: Renderable }
  })
...
export const Route = createFileRoute('/notes/')({
  ...
  loader: async ({ deps: { q } }) => {
    const { notes } = await getNotesRSC({ data: { q } })
    return { notes, q }
  },
})
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 text-center sm:text-left">
          ...
          <p className="mt-1 text-gray-500 dark:text-gray-400">
            Server Component RSC — data and rendering on the server
          </p>
        </div>
