...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try { await fetchCurrentUser(); } catch { /* offline — use login response user */ }
      await flushMutations();
      window.dispatchEvent(new CustomEvent("sw-auth-state-change", { detail: { authenticated: true } }));
      navigate("/notes");
    } catch (err: any) {
      ...
      <div className="w-full max-w-sm">
        <div className="rounded-xl border border-gray-200 bg-white p-8 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <div className="mb-6 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">to continue to the demo app</p>
          </div>
          ...
          <p className="mt-4 text-center text-xs text-gray-400 dark:text-gray-500">
            Demo: demo@swoff.dev / password123
          </p>
          <p className="mt-2 text-center text-sm text-gray-500 dark:text-gray-400">
            No account? <Link to="/register" className="text-teal-600 hover:text-teal-500 dark:text-teal-400">Register</Link>
          </p>
        </div>
      </div>
    </div>
