...
export default function NotesCreatePage() {
  ...
  const handleSubmit = async (data: Record<string, string>) => {
    ...
    if (!navigator.onLine) {
      await queueMutation({ method: "POST", url: "/api/notes", body, tags: generateTags("/api/notes") });
      navigate("/notes");
      return;
    }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8 dark:border-gray-700 dark:bg-gray-800">
          ...
          <NoteForm onSubmit={handleSubmit} onCancel={() => navigate("/notes")} submitLabel="Create Note" />
        </div>
      </div>
    </div>
  );
