...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try { await fetchCurrentUser(); } catch { /* offline */ }
      ...
      <div className="w-full max-w-sm">
        <div className="rounded-xl border border-gray-200 bg-white p-8 shadow-sm dark:border-gray-700 dark:bg-gray-800">
          <div className="mb-6 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">to start using the demo app</p>
          </div>
          ...
          <p className="mt-4 text-center text-sm text-gray-500 dark:text-gray-400">
            Already have an account? <Link to="/login" className="text-teal-600 hover:text-teal-500 dark:text-teal-400">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
