...
export default function TodosCreatePage() {
  ...
  const handleSubmit = async (data: Record<string, string>) => {
    ...
    if (!navigator.onLine) {
      await queueMutation({ method: "POST", url: "/api/todos", body, tags: generateTags("/api/todos") });
      navigate("/todos");
      return;
    }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="mx-auto max-w-2xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8 dark:border-gray-700 dark:bg-gray-800">
          <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-white">Create New Todo</h1>
          <TodoForm onSubmit={handleSubmit} onCancel={() => navigate("/todos")} submitLabel="Create Todo" />
        </div>
      </div>
    </div>
  );
